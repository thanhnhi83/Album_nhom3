...
/*style trong thông tin sản phẩm*/
.ProductInfo > .ProductVariable, .ProductQuantity, .AddToCart {
    padding-top: 10px;
    padding-bottom: 10px;
}
/*style trong chi tiết sản phẩm */
.ProductDetail * {
    padding-top: 10px;
}
/*chỉnh ảnh main */
...
<main class="container-fluid">
  <div class="row text-left">
    ...
    if ( isset( $_GET[ "id_product" ] ) ) {
        ...
        $controller->goiChiTietSanPham( $_GET[ "id_product" ] );
    }

    ?>
  </div>
  <div class="row text-white mx-3 border-top pt-3" style=' font-family: monospace;'>
    <h4>Sản phẩm khác:</h4>
    <br>
    <br>
    <?php
    if ( isset( $_GET[ "id_product" ] ) ) {
        ...
        $id = $_GET[ "id_product" ];
        $controller = new detail_controller();
        $controller->goiSanPhamKhac( $_GET[ "id_product" ] );
    }
    ?>
  </div>
